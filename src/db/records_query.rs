//! 记录查询引擎：records 表的查询参数模型、过滤 DSL、跨分片列表查询、
//! 详情行、分面统计与行 → JSON 映射。
//!
//! 从 `handlers::records_api` 抽取，消除 handler 层内嵌 SQL 的层次倒挂；
//! 本模块自包含，不依赖 handler 层。handler 通过本模块的 `pub(crate)` 接口执行查询。
...
/// 列表查询参数模型（axum `Query` 反序列化 + 过滤 DSL 输入）。
...
/// 分面统计：列出 records 中各维度非空去重取值（每维度最多 200 个）。
